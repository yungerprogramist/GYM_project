...
class TestExerciseAPI:
    ...
    def test_get_exercises_authorized(self, api_client, user, exercise):
        api_client.force_authenticate(user=user)
        response = api_client.get('/api/v1/exercises/')
        assert response.status_code == status.HTTP_200_OK

    def test_admin_can_create_exercise(self, api_client, admin_user, muscle_group):
    ...
    def test_regular_user_cannot_create_exercise(self, api_client, user, muscle_group):
        api_client.force_authenticate(user=user)
        data = {
            'name': 'Становая тяга',
