...
export interface Exercise {
  ...
  name: string;
  /**
   * Текст справа в плашке (например, "3 ПОДХ").
   * Опционально, потому что в исходном интерфейсе из задания этого поля нет.
   */
  rightLabel?: string;
  /**
   * Цветовая тема упражнения (фон карточки).
   * Опционально, потому что в исходном интерфейсе из задания этого поля нет.
   */
  color?: 'green' | 'purple' | 'blue' | 'pink';
  /**
   * Иконка упражнения (PNG/SVG).
   * Опционально, потому что в исходном интерфейсе из задания этого поля нет.
   */
  iconSrc?: string;
}
...
export interface ProgramDay {
  ...
  exercises: Exercise[];
  /**
   * Название дня ("Сила верха", "Сила низа"...). Для точного соответствия макету.
   */
  title?: string;
  /**
   * Цветные точки под заголовком дня (макет Frame 71).
   */
